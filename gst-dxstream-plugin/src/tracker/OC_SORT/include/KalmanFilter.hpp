...
class KalmanFilterNew {
  ...
  private:
    ...
    struct Data {
        ...
        //
        // 벡터의 **절대 길이**가 아니라 누적 push 횟수를 기억하는 이유: 이력은 이제 앞이
        // 잘려 나가므로 절대 길이가 시점마다 다른 것을 가리킨다. 해동은 "동결 뒤 몇 건이
        // 붙었나"라는 **차이**만 알면 되고, 그 차이는 잘라내기에 영향받지 않는다.
        //
        // 0 이 "동결 당시 아무것도 없었다" 와 "freeze() 가 없었다" 로 갈리지 않는 이유는
        // 이름이 바뀐 뒤에도 그대로다(리뷰 지적, e5ae47d): 유효성의 근거는 이 값이 아니라
        // 아래 IsInitialized 다. unfreeze() 는 그것을 먼저 확인하고 빠져나가므로, 여기까지
        // 왔다면 freeze() 가 반드시 선행했다.
